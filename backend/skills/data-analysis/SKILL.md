...
name: Data Analysis
description: >-
  数据分析方法论顾问（前置依赖）。在执行任何数据分析任务之前，必须先读取此 Skill，
  获取统计严谨性检查清单、常见分析陷阱（Simpson's Paradox、幸存者偏差等）和分析方法
  选择指南，以确保分析质量。此 Skill 是 duckdb-analysis、pandas-analysis 的前置方法论依赖，
  提供"分析前该怎么想"的框架，而非"怎么写代码"。补充参考文档：pitfalls.md、techniques.md。
---
